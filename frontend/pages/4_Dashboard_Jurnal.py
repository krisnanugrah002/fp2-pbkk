...
st.set_page_config(page_title="Dashboard Jurnal", page_icon="📝")
...
st.markdown("Catat dan pantau perkembangan berat badan, asupan makanan, serta aktivitas olahraga harianmu di sini.")
...
with tab_weight:
    ...
    with st.form("weight_log_form"):
        new_weight = st.number_input("Berat Badan (kg)", min_value=20.0, max_value=300.0, step=0.1)
        submit_weight = st.form_submit_button("Tambah Catatan Berat Badan")
        
        if submit_weight:
            res = create_weight_log_api(new_weight)
            if res and res.status_code == 200:
                st.success("Berat badan berhasil dicatat!")
                st.rerun()
            else:
                st.error("Gagal mencatat berat badan.")
...
with tab_food:
    st.subheader("Catat Konsumsi Makanan")
...
with tab_activity:
    st.subheader("Catat Aktivitas Olahraga")
        ...
        if submit_activity:
            if a_name.strip() == "":
                ...
                if res_activity and res_activity.status_code == 200:
                    ...
                    st.error("Gagal menyimpan data aktivitas.")
